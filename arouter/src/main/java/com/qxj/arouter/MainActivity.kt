 ...
 */
//在支持路由的页面上添加注解
//path路径至少需要两级 /xx/xx 其中com为分组标识  后面的为类标识
...
class MainActivity : AppCompatActivity() {
    ...
    private fun toSecondActivityOther() {
        //带参数的跳转
        ARouter.getInstance().build("/com/SecondActivity")
                .withString("name", "123")
                .withBoolean("is", true)
                .navigation()
    ...
    private fun toSecondActivityForResult() {
        //带返回值的跳转
        ARouter.getInstance().build("/com/SecondActivity").navigation(this, 10)
    ...
    private fun toSecondActivityCallback() {
        ARouter.getInstance().build("/com/SecondActivity").navigation(this, object : NavCallback() {
    ...
    private fun getFragment(): Fragment {
        val fragment = ARouter.getInstance().build("/com/OneFragment").navigation() as Fragment
        return fragment
    }
