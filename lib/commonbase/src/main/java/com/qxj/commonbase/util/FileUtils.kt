package com.qxj.commonbase.util
...
object FileUtils {
    ...
    /** * 创建文件 * * dirpath 文件目录 * fileName 文件名称 */
    ...
    /** * 创建文件 * filePath 文件路径 */
    private fun createFile(filePath: File) {
        if (!filePath.exists()) {
            filePath.createNewFile()
        }
    }

    /** * 删除文件 * * dirpath 文件目录 * fileName 文件名称 */
    ...
    /** * 删除文件 * filepath 文件路径 */
    private fun delFile(filepath: File): Boolean {
    ...
    private fun delDir(dirPath: String) {
        ...
        deleteDirWhiteFile(dir)
    }

    private fun deleteDirWhiteFile(dir: File?) {
        if (dir!!.checkFile())
            return
        for (file in dir.listFiles()) {
            if (file.isFile)
                ...
                deleteDirWhiteFile(file) // 递规的方式删除文件夹
        }
        dir.delete()// 删除目录本身
    }
    ...
    private fun copyFileTo(srcFile: File, destFile: File): Boolean {
    ...
    /** *拷贝目录下的所有文件到指定目录 * srcDir 原目录 * destDir 目标目录 */
    ...
    /** * 移动一个文件 */
    private fun moveFileTo(srcFile: File, destFile: File): Boolean {
    ...
    /** * 移动目录下的所有文件到指定目录 * srcDir 原路径 * destDir 目标路径 */
    @Throws(IOException::class)
